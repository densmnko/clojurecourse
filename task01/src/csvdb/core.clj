...
(defn key-value-pairs [tbl-keys tbl-record]
  ; or (flatten (seq (zipmap tbl-keys tbl-record))
...
(defn join* [data1 column1 data2 column2]
  ...
  (reduce
    (fn [coll x]
      (let [jk (get x column1)]
        (conj coll
              (merge (first (filter #(= jk (get % column2)) data2)) x ))))
    []
    data1))
